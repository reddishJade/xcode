...
## P1 · 删除 src/xcode/experimental，迁移 MCP 和 Memory，删除 Plugins
...
这与目标模块边界不一致。MCP 和 Memory 已确定为正式能力。Plugins 当前通过
`exec()` 在主进程中加载 `.local/plugins/*.py`，插件代码可直接访问文件系统、
网络和进程状态，不经过 ToolGate 或 PermissionEngine。它与 MCP、Skill 和内部
Hook API 的职责重叠，也没有隔离、依赖管理、兼容性协议或实际插件生态，因此
不应作为 Xcode 产品能力保留。
...
- 删除 `PluginManager`、`PluginManifest` 和动态 Python 文件加载实现。
- 删除 `plugins` enabled group、`.local/plugins/` 约定及其装配和测试。
- 删除 `plugins_hooks` 参数和仅为动态插件保留的 hook 注册路径；正式内部 hook
  API 不受影响。
...
均没有用户级 hook 注册、禁用、查看或诊断入口。当前唯一外部注入路径来自计划
删除的 Python Plugin 系统。
