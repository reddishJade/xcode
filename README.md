...
默认配置只启用 `core` 工具组。所有实验性能力必须显式 opt-in：可以单独启用某个 experimental group，也可以启用 `experimental` 总开关一次性打开全部实验性能力。
...
- **REPL 会话管理**：支持 `/plan`、`/review`、`/act`、`/compact`、`/sessions`、`/resume`、`/tool`、`@file` 引用和 session transcript 落盘。
- **可选扩展**：MCP、worktree、tasks、mailbox、progress、memory、plugins、daemon、speculation 都位于 `src/xcode/experimental/`，默认不加载。
...
详细模块说明见 [docs/code-organization.md](docs/code-organization.md)。
...
- `speculation`：构造 `SpeculationPlanner`
