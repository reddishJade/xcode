...
Agent Loop 的核心实现逻辑抽象后其实不到 30 行代码：
        ...
        tool_results = [
            {
                ...
                "content": await execute_tool(block.name, block.input),
            }
...
async def execute_load_skill(name: str) -> str:
    return await read_text(f"./skills/{name}.md")
```
...
好的做法用 `Pydantic` 模型把定义和实现绑在一起，参数描述直接约束格式，错误结构化给出修正建议：

```python
from pydantic import BaseModel, Field


class UpdateYuquePostInput(BaseModel):
    post_id: str = Field(description="语雀文章 ID，纯数字字符串，如 '12345678'")
    title: str | None = Field(default=None, description="文章标题，不改时可省略")
    content_markdown: str = Field(description="Markdown 格式正文")


async def update_yuque_post(input: UpdateYuquePostInput) -> dict:
    """更新语雀文章内容，不适合创建新文章。"""
    post = await get_post(input.post_id)
    if not post:
        raise ToolError(
            "文章 ID 不存在",
    ...
    return await update_post(input.post_id, input.title, input.content_markdown)
```
...
result = await run_agent_loop(task, messages=[])
...
# 写入：.team/inbox/{agent_id}.jsonl，append-only，崩溃可恢复
# 读取：按行解析，按 status 过滤
```
...
inbound = {"channel": channel, "session_key": session_key, "content": content}
...
class ChannelAdapter:
    ...
    async def send(self, session_key: str, text: str) -> None:
        ...
```
...
class AgentLoop:
    def __init__(self, bus, provider, workspace):
        ...
        self.tools = register_default_tools(workspace)  # shell、fs、web、message、cron
        self.sessions = SessionManager(workspace)       # 持久化会话历史
        self.memory = MemoryConsolidator(workspace, provider)  # 跨会话记忆整合

    async def run(self):
        while True:
            msg = await self.bus.consume_inbound()
            asyncio.create_task(self.dispatch(msg))  # 不 await：不同 session 的消息并发处理，互不阻塞

    async def dispatch(self, msg):
        session = self.sessions.get_or_create(msg.session_key)
        await self.memory.maybe_consolidate(session)  # token 超阈值时自动整合记忆

        messages = build_context(session.history, msg.content)
        result = await self.run_loop(messages)

        session.save(result["all_messages"])
        await self.bus.publish_outbound({"channel": msg.channel, "content": result["text"]})

    async def run_loop(self, messages):
        for _ in range(MAX_ITER):
            resp = await self.provider.chat(messages, self.tools.definitions())
            if resp.has_tool_calls:
                for call in resp.tool_calls:
                    result = await self.tools.execute(call.name, call.args)
                    messages = add_tool_result(messages, call.id, result)
            else:
                return {"text": resp.content, "all_messages": messages}  # 无工具调用，本轮结束


# 入口：接上渠道，启动
bus = MessageBus()
TelegramChannel(bus, allowed_ids=allowed_ids).start()  # Channel 只负责收发
asyncio.run(AgentLoop(bus, ClaudeProvider(), WORKSPACE).run())
```
...
class CronTask:
    id: str
    schedule: str  # cron 表达式，如 "0 9 * * 1-5"
    task: str      # 自然语言任务描述
    user_id: str   # 发结果给谁
...
async def save_progress(state: TaskState) -> None:
    path = Path(".openclaw/tasks") / f"{state.task_id}.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(asdict(state), ensure_ascii=False, indent=2), encoding="utf-8")


async def resume_task(task_id: str) -> TaskState | None:
    path = Path(".openclaw/tasks") / f"{task_id}.json"
    if not path.exists():
        return None  # 没有存档，从头开始
...
state = await resume_task(task_id)
# 有存档就从断点继续，没有就从头开始
```
...
```python
AUTHORIZED_USERS = {"user_id_tang", "user_id_other"}


async def handle_message(msg: InboundMessage) -> None:
    if msg.user_id not in AUTHORIZED_USERS:
        await send_reply(msg.user_id, "未授权")
        return
...
WORKSPACE = Path("/Users/tang/workspace").resolve()


async def execute_shell(args: list[str], cwd: str | None = None) -> str:
    # resolve 解析符号链接，relative_to 检查是否在工作空间内
    work_dir = Path(cwd or WORKSPACE).resolve()
    try:
        work_dir.relative_to(WORKSPACE)
    except ValueError as exc:
        raise ValueError(f"路径越界：{work_dir} 不在工作空间 {WORKSPACE} 内") from exc

    # 使用 create_subprocess_exec 而非 shell=True，避免 shell 注入
...
async def audited_shell(args: list[str], user_id: str) -> str:
    # 执行前记录：时间、用户、命令
    audit_line = json.dumps(
        {"timestamp": time.time(), "user_id": user_id, "command": " ".join(args)},
        ensure_ascii=False,
    )
    with Path(".openclaw/audit.jsonl").open("a", encoding="utf-8") as f:
        f.write(audit_line + "\n")
    return await execute_shell(args)
```
...
```python
providers = ["Anthropic", "OpenAI", "Anthropic Sonnet"]


async def run_with_fallback(task):
    for provider in providers:
        try:
            return await run_task(provider, task)
        except Exception:
