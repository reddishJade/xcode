...
Xcode 已完成 discovery、catalog、dedicated activation tool、资源路径披露、
session activation 生命周期和用户显式激活，已形成 model-driven 与
user-driven 共用状态的基础闭环。
...
MCP 已迁入 `harness/mcp/` 并由核心 runtime 自动发现配置；Memory 已迁入
`harness/memory/` 并保持独立 opt-in group。该迁移表示维护承诺和模块边界变化，
...
| Feature | 修正后状态 | 说明 |
| --- | --- | --- |
| Discovery paths | 已实现，当前是 5 级 | 显式配置目录优先，其后为 project `.xcode`、project `.agents`、user `.xcode`、user `.agents`；project 固定路径仅在 trusted workspace 配置启用时加入 |
| `SKILL.md` frontmatter | 已实现 | name 规范问题 warn + load；缺少 description 或不可恢复 YAML 时 skip；保留 license、compatibility、metadata、allowed-tools |
| Skill catalog | 已实现 | `SkillIndexCollector` 注入 name 和 description |
| Dedicated activation | 已实现 | `load_skill` 返回正文、skill root、资源相对路径和可恢复 activation 状态 |
| Reference loading | 已实现 | 可显式加载单个 `references/` 文件 |
| XML/binary/truncation safety | 已实现 | activation output 与 catalog 均执行 XML 转义；非法控制字符会替换，catalog description 限制为 768 字符 |
...
| `assets/` discovery | 已实现 | activation 结果列出 assets 相对路径，不主动读取内容 |
| Workspace trust config | 已实现 | `SkillsRuntimeConfig.trust_project_skills` 默认 false，未信任项目不披露项目级 skill |
| Model-driven activation | 已实现 | catalog 要求明确匹配 description 时先调用 `load_skill`；name schema 使用可见名称 enum |
| User-driven activation | 已实现 | `$skill-name` 和 `/skill skill-name` 共用 load_skill、PermissionEngine、history、resume 和 compaction 路径 |
...
- 用户级 discovery、显式信任后的项目级 discovery，以及最高优先级显式目录。
...
- activation 返回 skill root，并列出 `scripts/`、`references/` 和 `assets/`
  相对路径元数据，不主动读取或执行。
- session 内记录已激活 skill；重复 activation 返回简短状态。
...
- catalog 中稳定的 load-before-execution activation 指令。
- `load_skill.name` 的 discovered-name enum，以及空 registry 时不注册工具或
  catalog。
- `$skill-name` 和 `/skill skill-name` 显式入口、名称补全，以及未知、禁用、
  权限阻止状态诊断。
- name 字符集、长度、连续 hyphen 和目录一致性诊断；常见未引用冒号 YAML
  可恢复，完全不可解析时跳过。
- compatibility、license、metadata 和 allowed-tools 保留；activation 披露
  compatibility，allowed-tools 仅作为 advisory 且不能绕过 PermissionEngine。
...
- MCP 已迁入正式模块并由核心 runtime 自动发现；无配置时为空。
