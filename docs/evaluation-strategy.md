...
Harness Eval 不能假设模型在所有任务和预算下都具备足够的解题能力。每个外部任务至少
要经过模型、预算和最小对照的可解性校准，并把以下三类结果分开记录：

- **能力下限**：参考模型在声明预算下无法产生通过独立 verifier 的 patch；这描述模型、
  任务和预算的组合，不是 harness 归因结论；
- **Harness 增益**：同一模型、任务、预算和重复策略下，候选 harness 相对可运行对照的
  解决率、回归率和成本差异；
- **环境或任务失效**：provider、工具、工作区、verifier 或数据本身不能完成可判定运行。

预算不足必须单独标记。模型在较高预算下解决同一任务时，低预算失败不能被解释为模型
不可解；相反，如果参考模型在预算阶梯上始终失败，任务应进入“当前模型不可解”层，
而不是被包装成 harness 失败或成功。只有在任务集合包含足够的可解任务和有区分度的
失败任务时，Harness Eval 才能形成有意义的增益结论。
