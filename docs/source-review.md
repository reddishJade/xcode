...
`DelegatedTaskRunner` 控制并发和超时。`delegate_task` 是唯一暴露给父模型的
subagent 工具：调用即等待最终结果，子 agent 的 text/tool/result 事件通过
tool update 实时流向 REPL。支持 `worktree` 隔离。子 agent 使用过滤后的 tool
registry，且不包含 subagent 工具，因此当前只允许一层委托。
...
### Worktree backend (`harness/worktree.py`)
- `WorktreeTaskRunner`
- subagent `isolation="worktree"` 的正式依赖实现
- 管理工具 `create_worktree_task`、`remove_worktree_task`、`list_worktrees`、`prune_stale_worktrees`
  作为正式 `worktree` 工具组默认暴露
