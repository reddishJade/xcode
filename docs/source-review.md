...
不存在 aggregate experimental switch。MCP 由核心 runtime 自动发现配置；
Memory 通过独立的 `memory` group 启用。
...
- 双向 JSON-RPC 消息分类、ping 和 method-not-found 响应
- `.local/mcp_cache.json` schema cache
- `defer_loading` bootstrap/search flow
- 动态 `mcp__{server}__{tool}` 注册
- **未实现**: remote transports (SSE, HTTP, Streamable HTTP), OAuth, resources, prompts, listChanged、特定 notification 处理、非文本 attachment
...
2. 不要默认启用扩展能力。
3. 不新增 `experimental` package 或 aggregate switch；可选能力使用独立 group。
