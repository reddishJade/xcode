"""隐藏 verifier：检查 observer 后台调度和稳定 hook 回归。"""
...
def main() -> int:
    ...
    regression_file = workspace / "src/xcode/tests/test_xcode_external_hooks.py"
    regression = _run(
        (
            sys.executable,
            "-m",
            ...
            (
                str(regression_file)
                + "::ExternalHookRunnerTests"
                + "::test_executes_json_hook_and_redacts_input"
            ),
            (
                str(regression_file)
                + "::ExternalHookRunnerTests"
                + "::test_pre_tool_deny_blocks_handler"
            ),
            *common,
        ),
        workspace=workspace,
