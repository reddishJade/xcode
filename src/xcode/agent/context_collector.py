...
SKILLS_MAX_BYTES: int = 16 * 1024  # 16KB —— 技能总输出上限
SKILLS_MAX_FILE_BYTES: int = 64 * 1024  # 64KB —— 单文件跳过阈值
_SKILLS_TRUNCATED_MARKER = (
    "<skills-truncated>Skills truncated. "
    "Activate fewer skills or reduce individual skill sizes.</skills-truncated>"
)

_SKILLS_ALLOWED_SUFFIXES: frozenset[str] = frozenset({".md", ".mdx", ".txt"})
SKILLS_MAX_FILES: int = 50  # 单目录最多扫描文件数
SKILLS_MAX_DEPTH: int = 3  # 最大目录递归深度


def _walk_skill_files(skill_dir: Path) -> list[Path]:
    """带边界约束的递归遍历：限制深度、跳过符号链接目录、限制文件总数。"""
    files: list[Path] = []

    def _walk(current: Path, depth: int) -> None:
        if depth > SKILLS_MAX_DEPTH:
            return
        if len(files) >= SKILLS_MAX_FILES:
            return
        try:
...
def _is_child_path(path: Path, base_dir: Path) -> bool:
    """检查 path 的解析后路径是否在 base_dir 之下，防止符号链接路径遍历。"""
    try:
...
class SkillCollector:
    """从项目级和用户级技能目录收集显式声明的技能文件。

    按优先级搜索目录:
      1. <project>/.xcode/skills/
      2. <project>/.agents/skills/
      3. ~/.xcode/skills/
      4. ~/.agents/skills/

    同一相对路径在高优先级目录中存在时优先覆盖。
    输出按 (优先级, 相对路径) 确定性排序。
    忽略缺失目录、超大文件、禁止后缀和路径遍历攻击。

    优先级 MEDIUM：技能是辅助指令，非即时关键上下文。
    大小限制 SKILLS_MAX_BYTES（16KB），超出时截断。
    失败时记录日志并返回 []。
    """
    ...
    def __init__(
        self,
        project_root: Path | None = None,
        max_bytes: int = SKILLS_MAX_BYTES,
    ) -> None:
        self._project_root = project_root
        self._max_bytes = max_bytes
    ...
    def collect(self, input: ContextCollectionInput) -> list[ContextBlock]:
        root = input.project_root or self._project_root
        dirs = _build_skill_search_dirs(root)

        seen: dict[str, tuple[Path, int]] = {}

        for search_dir, group in dirs:
            if not search_dir.is_dir():
                continue
            try:
                for file_path in _walk_skill_files(search_dir):
                    if file_path.suffix.lower() not in _SKILLS_ALLOWED_SUFFIXES:
                        continue
                    if file_path.stat().st_size > SKILLS_MAX_FILE_BYTES:
                        continue
                    if not _is_child_path(file_path, search_dir):
                        continue
                    rel = file_path.relative_to(search_dir).as_posix()
                    if rel in seen:
                        continue
                    seen[rel] = (file_path, group)
            except Exception:
                logger.exception("SkillCollector: failed to scan %s", search_dir)
                continue

        if not seen:
            return []

        sorted_items = sorted(seen.items(), key=lambda x: (x[1][1], x[0]))

        parts: list[str] = []
        total_bytes = 0
        marker = _SKILLS_TRUNCATED_MARKER

        for rel_path, (file_path, _group) in sorted_items:
            try:
