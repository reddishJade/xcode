...
class DeepSeekProvider:
    ...
    def complete(
        ...
        **kwargs: Any,
        ...
        """DeepSeek non-streaming chat completion."""
        # Clean invalid parameters in thinking mode
        if self.thinking:
        ...
        if tools:
            strict_tools = kwargs.pop(
                "strict_tools", getattr(self, "strict_tools", False)
            )
            params["tools"] = [
                to_chat_tool(
                    getattr(t, "name", ""),
        ...
        # Retry once if JSON mode is requested but response content and tool calls are both empty.
        # Note: This retry mechanism assumes an "occasional empty response" from DeepSeek API
        # (e.g. temporary server-side overloads during peak times), not a deterministic rejection.
        is_empty_res = not getattr(message, "content", "") and not getattr(
