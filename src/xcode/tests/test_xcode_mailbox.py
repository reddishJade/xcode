...
class TestMailboxExpiryAndAckSeparation:
    ...
    def test_migration_moves_legacy_acks(self) -> None:
        """主 JSONL 中的 legacy ack 行被迁移到 .ack 文件。"""
        recipient = "b"
        main_path = mailbox_inbox(self.root) / f"{recipient}.jsonl"
        main_path.parent.mkdir(parents=True, exist_ok=True)
        # 手动构造 legacy 格式：message 和 ack 混在主文件
        ...
        mailbox = AgentMailbox(self.root)
        # 触发迁移（通过 read）
        unread = mailbox.read_unread_messages(recipient)
        # m1 已被 ACK，不应出现在 unread
        assert all(m["message_id"] != "m1" for m in unread)
        # 主文件不再含 ack 行
        main_lines = [
            line for line in main_path.read_text(encoding="utf-8").splitlines() if line
        ]
        assert all(json.loads(line).get("event") == "message" for line in main_lines)
        # .ack 文件含迁移的 ack
        ack_path = mailbox_inbox(self.root) / f"{recipient}.ack"
        assert ack_path.exists()
        ack_lines = [
            line for line in ack_path.read_text(encoding="utf-8").splitlines() if line
        ]
        assert any(json.loads(line).get("message_id") == "m1" for line in ack_lines)
        # marker 文件存在
        marker = mailbox_inbox(self.root) / f"{recipient}.migrated_v2"
        assert marker.exists()

    def test_migration_idempotent(self) -> None:
        """迁移只执行一次，marker 存在时不再处理。"""
        recipient = "b"
        mailbox = AgentMailbox(self.root)
        marker = mailbox_inbox(self.root) / f"{recipient}.migrated_v2"
        marker.parent.mkdir(parents=True, exist_ok=True)
        marker.touch()
        # 构造主文件含 ack 行（模拟未迁移状态），但 marker 已存在
        main_path = mailbox_inbox(self.root) / f"{recipient}.jsonl"
        main_path.write_text(
            json.dumps({"event": "ack", "message_id": "x", "ack_at": "now"}) + "\n",
            encoding="utf-8",
        )
        # read 触发 _migrate_legacy_acks，但因 marker 存在应跳过
        mailbox.read_unread_messages(recipient)
        # 主文件未被重写（ack 行仍在）
        main_lines = [
            line for line in main_path.read_text(encoding="utf-8").splitlines() if line
        ]
        assert any(json.loads(line).get("event") == "ack" for line in main_lines)
