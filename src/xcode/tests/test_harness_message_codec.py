"""Compacted dict ↔ AgentMessage 编解码单元测试。"""
...
class TestMessagesFromCompactedDicts:
    ...
    def test_user_message(self) -> None:
        result = messages_from_compacted_dicts([{"role": "user", "content": "hello"}])
        assert len(result) == 1
    ...
    def test_unknown_role_returns_none(self) -> None:
        result = messages_from_compacted_dicts([{"role": "unknown", "content": "x"}])
        assert len(result) == 0


class TestToolCallFromCompacted:
    def test_valid(self) -> None:
        item = {"id": "c1", "function": {"name": "search", "arguments": '{"q": "x"}'}}
        result = _tool_call_from_compacted(item)
    ...
    def test_missing_id_returns_none(self) -> None:
        ...
        assert _tool_call_from_compacted(item) is None
    ...
    def test_not_dict_returns_none(self) -> None:
        assert _tool_call_from_compacted("string") is None
