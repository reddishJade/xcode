...
class TestEstimateTokens(unittest.TestCase):
    ...
    def test_extract_prompt_tokens(self):
        ...
        self.assertEqual(extract_prompt_tokens_from_usage(usage), 1000)

    def test_extract_prompt_tokens_none(self):
        """测试 usage 为 None。"""
        self.assertIsNone(extract_prompt_tokens_from_usage(None))
        self.assertIsNone(extract_prompt_tokens_from_usage({}))


class TestModelSoftThreshold(unittest.TestCase):
    """测试模型软阈值。"""

    def test_deepseek_threshold(self):
        """测试 DeepSeek 模型阈值。"""
        self.assertEqual(get_model_soft_threshold("deepseek-v4-flash"), 60000)
        self.assertEqual(get_model_soft_threshold("deepseek-v3-pro"), 60000)
        self.assertEqual(get_model_soft_threshold("deepseek-v2"), 28000)

    def test_chatglm_threshold(self):
        """测试 ChatGLM 模型阈值。"""
        self.assertEqual(get_model_soft_threshold("glm-4.7"), 120000)
        self.assertEqual(get_model_soft_threshold("glm-5"), 120000)

    def test_mimo_threshold(self):
        """测试 MiMo 模型阈值。"""
        self.assertEqual(get_model_soft_threshold("mimo-v2.5-pro"), 60000)

    def test_gpt_threshold(self):
        """测试 GPT 模型阈值。"""
        self.assertEqual(get_model_soft_threshold("gpt-4-turbo"), 120000)
        self.assertEqual(get_model_soft_threshold("gpt-3.5-turbo"), 14000)

    def test_default_threshold(self):
        """测试未知模型使用默认阈值。"""
        self.assertEqual(get_model_soft_threshold("unknown-model"), 32000)
        self.assertEqual(get_model_soft_threshold(None), 32000)


class TestShouldCompactTokenAware(unittest.TestCase):
    """测试 token-aware 压缩触发。"""
